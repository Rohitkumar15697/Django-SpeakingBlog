...
def profile(request):
    ...
    if request.method=='POST':
        ...
        if fm.is_valid():
            ...
            obj=blogpost(topic=topic,title=title,post=post,date=datetime.today())
    ...
    postdata=blogpost.objects.all().filter(created_by=request.user).order_by('-date')
    
    postdata=list(postdata)     #Convert all objects to list
    count=len(postdata)
